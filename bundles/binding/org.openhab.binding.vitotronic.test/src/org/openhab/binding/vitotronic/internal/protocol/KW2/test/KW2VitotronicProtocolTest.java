...
import org.openhab.binding.vitotronic.internal.protocol.KW2.*;
import org.openhab.binding.vitotronic.internal.protocol.utils.*;

import static org.mockito.Mockito.*;

public class KW2VitotronicProtocolTest {
	...
	public void testGetByteQueueForInit() {
		IVitotronicProtocol testObject = new KW2VitotronicProtocol(unconfiguredParameterFactory());
	...
	private IParameter configureParameter() {
		...
		when(parameterMock.addressSize()).thenReturn(2);
		when(parameterMock.dataSize()).thenReturn(2);
	...
	public void testExpectedReadingParameterResponseSize() {
		...
		verify(parameter).addressSize();
		verify(parameter).dataSize();
	}
	...
	public void testParseReadParameterResponse() {
		IVitotronicProtocol testObject = new KW2VitotronicProtocol(configuredParameterFactory());
	...
	private IParameterFactory configuredParameterFactory() {
		...
		when(parameterFactory.createParameterFor(0x00F8)).thenReturn(parameter);
		when(parameter.dataSize()).thenReturn(2);
