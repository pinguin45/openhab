 ...
 * @since 1.0.0
 */
public class OnePhasePLCBusProtocol implements IPLCBusProtocol {
	...
	public ActorResponse parseResponseFromActor(IByteQueue receivedBytes) {
		...
		ReceiveFrame receiveFrame = getReceiveFrameOf(receivedBytes);
		
		if (receiveFrame.isItself()) {
			receiveFrame = getReceiveFrameOf(receivedBytes);
		}
		...
		return new ActorResponse
				(
						receiveFrame.isAcknowledgement(),
						receiveFrame.getCommand(),
						receiveFrame.getFirstParameter(),
						receiveFrame.getSecondParameter()
				);
	}
